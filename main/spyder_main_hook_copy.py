# -*- coding: utf-8 -*-
"""
Created on Tue Jun 20 17:41:14 2023
...
SCALING = 1
ARUCO_DICT = cv.aruco.DICT_5X5_50  
aruco_scale = 46.0
...
aruco_edges3 = np.array([[0, 293.2, 0], [aruco_scale, 293.2, 0], [aruco_scale, 293.2 + aruco_scale, 0], 
                         [0, 293.2 + aruco_scale, 0]],
                        dtype='float32').reshape((4, 1, 3))

aruco_edges4 = np.array( [[136.5 + aruco_scale, 293.2, 0], [136.5 + 2 * aruco_scale, 293.2, 0],
                          [136.5 + 2 * aruco_scale, 293.2 + aruco_scale, 0], 
                          [136.5 + aruco_scale, 293.2 + aruco_scale, 0]],dtype='float32').reshape((4, 1, 3))

aruco_edge_array = [aruco_edges1, aruco_edges2, aruco_edges3, aruco_edges4] #corners of aruco edges on paper


#takes 2D image points & returns the projected 2D image points (points), rotation vector (rvec), 
    #and translation vector (tvec) if the pose estimation was successful.
...
def main():
    ...
    fns = glob.glob('\PycharmProjects\pythonProject2\main\hook_data\OnePlus\Photos_tele_aruco*')
...
def test_multi_aruco(index):
    ...
    fns += glob.glob('C:/Users/nicko/PycharmProjects/pythonProject2/main/hook_data/aruco_test2/1*')
    fns += glob.glob('C:/Users/nicko/PycharmProjects/pythonProject2/main/hook_data/aruco_test1/1*')
    fns += glob.glob('C:/Users/nicko/PycharmProjects/pythonProject2/main/hook_data/aruco_test1/2*')
    fns.sort()
