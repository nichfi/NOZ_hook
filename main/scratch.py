...
def trimesh(camera_origin=None, contour_points=None):
    ...
    # Create and add the surface mesh item
    mesh = gl.GLMeshItem(
        vertexes=vertices,
        faces=triangles,
        edgeColor=[0, 0, 0, 1],
    ...
    for i in np.arange(len(points)):
        ...
        try:
            ...
            break
        trianglesB.append([0, len(pB) - 1, len(pB)])

    trianglesB.append([0, 1, len(pB) - 1])
    verticesB = np.array(pB)
    trianglesB = np.array(trianglesB)

    # Add the second surface mesh item
    meshB = gl.GLMeshItem(
        vertexes=verticesB,
        faces=trianglesB,
        edgeColor=[1, 0, 0, 1],
